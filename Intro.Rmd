...
```{r echo=TRUE}
set.seed(6)  #Semilla
a <- sample(385792,16948,replace=F);a  #Obtiene una muestra aleatoria del tamaño recomendado en chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/https://www.inegi.org.mx/contenidos/programas/enoe/15ymas/doc/enoe_n_diseno_muestral.pdf (numero de obs. de la pob.,tamaño de la muestra,remplazo)
m <- sort(a);m  #Ordena los resultados de menor a mayor

submuestra <- ENOET113[m,]; View(submuestra)
```
...
```{r echo=TRUE}
set.seed(7)  #Semilla
b <- sample(385792,16948,replace=F);a  #Obtiene una muestra aleatoria (numero de obs. de la pob.,tamaño de la muestra,remplazo)
n <- sort(b);n  #Ordena los resultados de menor a mayor

submuestra2 <- ENOET113[n,]; View(submuestra2)
```
```{r echo=TRUE}
